...
# Ойын терезесінің (экранының) параметрлері
class Window:
    ...
    # Блоктың белгіленген шектен шығу-шықпауын тексеретін әдіс
...
class Snake:
    ...
    def change_direction(self, new_direciton: Direction) -> None:
        # Жылан бірден қарама-қарсы бағытта қозғала алмайды
...
class Game:
    ...
    SNAKE_X_POS = 100
    SNAKE_Y_POS = 50
    # Ойын терезесінің ені мен биіктігі
    WINDOW_WIDTH = 720
    WINDOW_HEIGHT = 480
    # Жылан ұзындығы
    SNAKE_LENGTH = 3
    ...
    def __init__(self, difficulty: Difficulty) -> None:
        ...
        # FPS - 1 секундтағы кадрлар сандарының бақылаушылары
        self.fps_controller = pygame.time.Clock()

        self.window_frame = Window(Game.WINDOW_WIDTH, Game.WINDOW_HEIGHT)

        #  Түс объекттері. Color клаcтың конструкторына берілетін 3 сан - қызыл, жасыл, көк (RGB color model) түстерінің кодтары. Осылайша түстерді анықтай аламыз.
        ...
        self.score = 0
        # Тамақтың координаталары өзгергенін анықтайды
    ...
    def game_over(self) -> None:
        ...
        # Ойын аяқталды мәтін төртбұрышының орны
        game_over_rect = game_over_surface.get_rect()
        game_over_rect.midtop = self.window_frame.get_top_location()
        # Ойын терезесін қара түспен бояйды
        self.game_window.fill(self.black)
        self.game_window.blit(game_over_surface, game_over_rect)
        # ойын ұпайының суретін салу
    ...
    def paint_snake(self) -> None:
        for pos in self.snake.snake_body:
            # Жылан денесі
            # pygame.draw.rect(ойын_беті, түс, (x,y-координаталары))
            # (x,y-координаталары) -> pygame.Rect(x, y, x_өлшемі, y_өлшемі)
            pygame.draw.rect(
                self.game_window,
                self.green,
    ...
    def start(self) -> None:
        ...
        while True:
            ...
            # Экранда жаңа тамақтың пайда болуы
            if self.food_spawn:
                self.food_pos = Block.create_random_block(
                    self.window_frame.frame_size_x,
                    self.window_frame.frame_size_y,
...
def main() -> None:
    ...
    print("Ойын қиыншылығын таңданыз")
    print("Қиындық тандау үшін 1-4 сан таңдаңыз")
    print("Оңай      ->  1")
    print("Орташа    ->  2")
    print("Қиын      ->  3")
    print("Өте қиын  ->  4")
    ...
    while not difficulty in ["1", "2", "3", "4"]:
        print("1 мен 5 арасындағы санды таңдаңыз")
        difficulty = input()
